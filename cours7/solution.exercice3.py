# Voici une liste de recettes. 
# Imprime à l'écran chaque recette répondant aux critères suivants :

#   1. Que l'on affiche la liste des recettes qui :
#       1.1. inclus de la mozzarella sans que la recette soit de la Pizza
#       1.2. inclus du sel, avec une difficulté modéré
#       1.3. inclus du poulet et qui prend au moins 20 minutes à préparer.

#   2. Les informations afficher à l'écran doivent être :
#       2.1. le nom de la recette
#       2.2. le temps de préparation
#       2.3. le temps de cuisson
#       2.4. la liste des ingrédients
#       2.5. la note de la recette 

# INDICES :
# 1. Ne pas oublier les indices des exercices 1 et 2 pour filtrer les informations.
# 2. Pour cherche un mot dans une liste, il suffit de faire : 
#    "Salad" in name 
#    pour chercher si le mot "Salad" est contenu dans la variable name.
...
# Voir exercice #1 pour comprendre l'utilisation de cette variable.
is_good_recipe = False

# Comme pour l'exercice #2, on regarde chaque recette individuellement.
for recipe in recipes :

  # Si le nom de la recette ne contient pas le mot pizza, on regarde chaque ingrédient
  # pour savoir si la recette contient de la mozzarella. Si oui, on veut afficher la recette. 
  if(not ("pizza" in recipe["name"].lower())) :
    for ingredient in recipe["ingredients"] :
      if ("mozzarella" in ingredient.lower()) :
          is_good_recipe = True
  
  # Si la recette est de difficulté modéré, on regarde chaque ingrédient
  # pour savoir si la rectte contient du sel. Si oui, on veut afficher la recette. 
  if (recipe["difficulty"] == "Medium") :
    for ingredient in recipe["ingredients"] :
      if ("salt" in ingredient.lower()) :
          is_good_recipe = True
  
  # Si la recette prend au moins 20 minutes à préparer, on regarde chaque ingrédient
  # pour savoir si la recette contient du poulet. Si oui, on veut afficher la recette.
  if (recipe["prepTimeMinutes"] >= 20) :
    for ingredient in recipe["ingredients"] :
      if ("chicken" in ingredient.lower()) :
          is_good_recipe = True

  # Si la recette rencontre les critères précédent, on affiche plusieurs informations de
  # de cette recette avec une mise en page. 
  if(is_good_recipe) :
    print("_________________________________________________________")
    print("                                                         ")
    print("Name :",recipe["name"])
    print("Preparation Time in Minutes :", recipe["prepTimeMinutes"])
    print("Cooking Time in Minutes :", recipe["cookTimeMinutes"])
    print("Ingredients :")
    for ingredient in recipe["ingredients"] :
       print("  ",ingredient)
    print("Rating:",recipe["rating"])

  # Avant de passer à la recette suivante, il ne faut pas oublier de réinitialiser la
  # variable a Faux. Sinon, chaque recette après la première recette Vrai seront tous Vrai.
  is_good_recipe = False
